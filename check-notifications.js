...
async function checkData() {
  ...
  if (anonError) {
    ...
    const enabled = anonUsers.filter(u => u.pushToken && u.notificationsEnabled);
    console.log("  - With push tokens:", withTokens.length);
    console.log("  - With tokens & enabled:", enabled.length);
    if (enabled.length > 0) {
      console.log("  - Sample token:", enabled[0].pushToken?.substring(0, 30) + "...");
    }
  }
  ...
  if (notifError) {
    ...
    if (analyticsError) {
      ...
      analytics.forEach(a => {
        console.log(`  - ${a.event_type}: ${a.device_count} devices at ${a.timestamp}`);
      });
    }
  }
}
